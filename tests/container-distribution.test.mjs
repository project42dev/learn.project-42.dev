...
test("self-host image is non-root and uses the supported public configuration", async () => {
  ...
  assert.match(
    ...
    new RegExp(`^(?:github:project42dev/project42-platform|git\\+https://github\\.com/project42dev/project42-platform\\.git)#v${compatibility.platform.requiredVersion}import assert from "node:assert/strict";
import { readFile } from "node:fs/promises";
import { test } from "node:test";
